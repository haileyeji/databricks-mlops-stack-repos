...
The databricks CLI bundles top level is defined by file `yjlee_mlops_on_bricks/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`yjlee_mlops_on_bricks/resources/model-workflow-resource.yml`)
 - batch inference workflow (`yjlee_mlops_on_bricks/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`yjlee_mlops_on_bricks/resources/monitoring-resource.yml`)
 - feature engineering workflow (`yjlee_mlops_on_bricks/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`yjlee_mlops_on_bricks/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `yjlee_mlops_on_bricks/databricks.yml` and `yjlee_mlops_on_bricks/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-yjlee_mlops_on_bricks-model     | /dev-yjlee_mlops_on_bricks-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-yjlee_mlops_on_bricks-model | /staging-yjlee_mlops_on_bricks-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-yjlee_mlops_on_bricks-model    | /prod-yjlee_mlops_on_bricks-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-yjlee_mlops_on_bricks-model    | /test-yjlee_mlops_on_bricks-experiment    |
...
2. Complete the `TODO` in `yjlee_mlops_on_bricks/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `yjlee_mlops_on_bricks/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `yjlee_mlops_on_bricks/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-yjlee_mlops_on_bricks-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-yjlee_mlops_on_bricks-batch-inference-job`
that runs the notebook under `yjlee_mlops_on_bricks/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-yjlee_mlops_on_bricks-batch-inference-job`.
...
`test-yjlee_mlops_on_bricks-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-yjlee_mlops_on_bricks-batch-inference-job`.
